...
- **Real callers:** Only `chittycommand/.github/workflows/getchitty-creds-example.yml` (example). `chittyops/.github/workflows/reusable-worker-deploy.yml` does NOT use the action — uses static `CLOUDFLARE_API_TOKEN`. Behavior changes are safe to ship.
